...
// webs-components:/Users/conradklek/webs/packages/webs-site/src/app/irc/[channel].webs
import { useTable as useTable2, state as state15, onReady as onReady2, watch as watch2, session as session3 } from "@conradklek/webs";
var __channel__default = {
  ...
  template: `<div class="chat-container">
    ...
    <div class="message-list">
      {#each messages.state.data as msg}
  ...
  setup(props, ctx) {
    ...
    const messages = useTable2("chat_messages");
    const currentMessage = state15("");
    const messageListEl = state15(null);
    onReady2(() => {
    ...
    const handleSend = async () => {
      ...
      if (msg && session3.user) {
        await messages.put({
          id: crypto.randomUUID(),
          channel: formattedChannelName,
          username: session3.user.username,
          message: msg,
          user_id: session3.user.id,
          created_at: new Date().toISOString()
        });
        currentMessage.value = "";
      }
    };
    watch2(() => messages.state.data, () => {
      setTimeout(() => {
        if (messageListEl.value) {
          messageListEl.value.scrollTop = messageListEl.value.scrollHeight;
        }
