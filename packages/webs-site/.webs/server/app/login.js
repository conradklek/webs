...
// webs-components:/Users/conradklek/webs/packages/webs-site/src/app/login.webs
import { session as session3, state as state15, router, onReady as onReady2 } from "@conradklek/webs";
var login_default = {
  name: "login",
  template: `<form
  ...
  setup() {
    const email = state15("");
    const password = state15("");
    const error = state15(null);
    onReady2(() => {
      if (session3.isLoggedIn) {
        router.push(`/${session3.user.username}`);
      }
    });
    async function handleLogin() {
      error.value = null;
      try {
        const user = await session3.login(email.value, password.value);
    ...
    return { email, password, error, handleLogin, session: session3 };
  }
};
export {
