...
// webs-components:/Users/conradklek/webs/packages/webs-site/src/gui/todo-list.webs
import {
  state as state15,
  session as session3,
...
var todo_list_default2 = {
  ...
  setup(props) {
    const newTodoContent = state15("");
    const todos = useTable2("todos", props.initialState?.initialTodos);
    const sortedTodos = computed6(() => [...todos.data || []].sort((a, b) => new Date(b.created_at) - new Date(a.created_at)));
    function createTodo() {
      const content = newTodoContent.value.trim();
      if (!content || !session3.user?.id)
        return;
      todos.put({
        id: crypto.randomUUID(),
        content,
