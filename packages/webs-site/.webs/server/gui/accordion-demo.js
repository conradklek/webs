...
// webs-components:/Users/conradklek/webs/packages/webs-site/src/gui/accordion.webs
...
var AccordionItem3 = {
  ...
  setup(props) {
    provide11("itemValue", props.value);
  },
  template: `
        <div class="w-full flex flex-col gap-1.5">
          <slot></slot>
...
var AccordionTrigger3 = {
  name: "accordion-trigger",
  setup() {
    const accordion = inject11("accordion");
    const { toggle } = accordion || {};
    const value = inject11("itemValue");
    return {
      handleClick: () => toggle && value && toggle(value)
    };
  },
...
var AccordionContent3 = {
  name: "accordion-content",
  setup() {
    const accordion = inject11("accordion");
    const { openItems } = accordion || {};
    const value = inject11("itemValue");
    const isOpen = computed6(() => openItems && openItems.has(value));
    return { isOpen };
  },
  template: `
        {#if isOpen}
...
var accordion_default3 = {
  name: "accordion",
  template: "",
  style: "",
  name: "accordion",
  components: {
    "accordion-item": AccordionItem3,
    "accordion-trigger": AccordionTrigger3,
    "accordion-content": AccordionContent3
  },
  props: {
    type: {
      type: String,
  ...
  setup(props) {
    const openItems = state15(new Set);
    function toggle(value) {
      const newSet = new Set(openItems);
      if (props.type === "single") {
        if (newSet.has(value)) {
...
// webs-components:/Users/conradklek/webs/packages/webs-site/src/gui/accordion-demo.webs
var accordion_demo_default2 = {
  name: "accordion-demo",
  template: `<accordion type="single" collapsible>
    <accordion-item value="item-1">
      <accordion-trigger>Is it accessible?</accordion-trigger>
  ...
  components: {
    ...registry_default,
    accordion: accordion_default3,
    ...accordion_default3.components
  }
};
export {
  accordion_demo_default2 as default
};
