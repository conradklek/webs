...
function createTableSql(tableName, table) {
  ...
  const foreignKeys = Object.entries(table.fields)
    ...
    .map(([fieldName, props]) => {
      ...
      if (props.onDelete) {
        fkSql += ` ON DELETE ${props.onDelete}`;
      }
      return fkSql;
    });
...
function createActionsFileContent(syncableTables) {
  const actions = syncableTables
    .map(([tableName, table]) => {
      ...
      // UPDATED: Exclude content blobs from server-side upserts
      ...
      ON CONFLICT(${table.keyPath}) DO UPDATE SET
      ${fieldNames
    ...
    `
      ...
      // UPDATED: Ensure delete respects user_id for security
...
export async function createDatabaseAndActions(
  ...
  if (!dbConfig?.name) {
    console.error('Database file name not specified.');
    process.exit(1);
  }
