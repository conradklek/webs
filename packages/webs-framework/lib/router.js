...
async function executePrefetch(routeDefinition, context) {
  log(`Executing prefetch for component '${routeDefinition.componentName}'...`);
  ...
  try {
    ...
    error(`Prefetch Error for "${routeDefinition.componentName}":`, err);
...
export function createFetchHandler(context) {
  ...
  return async function fetch(req, server) {
    ...
    if (req.headers.get('upgrade') === 'websocket') {
      ...
      if (pathname === '/api/chat') return ai.handleChatUpgrade(req);
      ...
      try {
        ...
        if (pathname.startsWith('/api/ai/models/')) {
          ...
          if (pathname.endsWith('/pull') && req.method === 'POST') {
            ...
            return new Response(await ai.pull(model), {
              headers: { 'Content-Type': 'application/json; charset=utf-8' },
            });
          }
