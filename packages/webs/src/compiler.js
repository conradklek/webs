...
export function parse_markdown(md_str) {
  ...
  const _parse_inline = (txt, ctx) => {
    ...
    const inline_rules = [
      { regex: /!\[(.*?)\]\((.*?)\)/g, replacer: (alt, src) => `<img src="${src}" alt="${alt}">` },
      { regex: /\[(.*?)\]\((.*?)\)/g, replacer: (text, href) => `<a href="${href}">${text}</a>` },
      { regex: /\*{3}(.*?)\*{3}|_{3}(.*?)_{3}/g, replacer: (m1, m2) => `<strong><em>${m1 || m2}</em></strong>` },
      { regex: /\*{2}(.*?)\*{2}|_{2}(.*?)_{2}/g, replacer: (m1, m2) => `<strong>${m1 || m2}</strong>` },
      { regex: /(?<![a-zA-Z0-9])\*(.*?)\*(?![a-zA-Z0-9])|(?<![a-zA-Z0-9])_(.*?)_(?![a-zA-Z0-9])/g, replacer: (m1, m2) => `<em>${m1 || m2}</em>` },
      { regex: /~~(.*?)~~/g, replacer: (text) => `<del>${text}</del>` },
    ];

    for (const rule of inline_rules) {
      proc_txt = proc_txt.replace(rule.regex, (_, ...args) => rule.replacer(...args.slice(0, rule.replacer.length)));
    }
  ...
  const _parse_fenced_code = (lns) => {
    ...
    const code = code_lns.join('\n').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const lang_class = lang ? ` class="language-${lang}"` : '';
    return { html: `<pre><code${lang_class}>${code}</code></pre>`, consumed: i + 1 };
  };
  ...
  const _parse_table = (lns, ctx) => {
    if (lns.length < 2 || !lns[0].includes('|') || !/^\s*\|?.*:?-+.*\|?\s*$/.test(lns[1])) return null;
    const headers = lns[0].split('|').map(h => h.trim()).filter(Boolean);
    const aligns = lns[1].split('|').map(s => s.trim()).filter(Boolean).map(s => {
    ...
    for (let i = 2; i < lns.length; i++) {
      ...
      const cells = lns[i].split('|').map(c => c.trim()).filter(Boolean);
  ...
  const _parse_list = (lns, ctx) => {
    ...
    while (i < lns.length) {
      ...
      let li_content;
      if (task_match) {
        const checkbox = `<input type="checkbox" ${task_match[1] === 'x' ? 'checked' : ''} disabled> `;
        const text_to_parse = content_lns.join('\n').substring(content_lns[0].indexOf(task_match[2]));
        li_content = checkbox + _parse_blocks(text_to_parse.split('\n'), ctx).replace(/^<p>|<\/p>$/g, '');
      } else {
        li_content = _parse_blocks(content_lns, ctx);
      }
      items.push(`<li>${li_content}</li>`);
      i = j;
    }
    return { html: `<${tag}${start}>${items.join('\n')}</${tag}>`, consumed: i };
  };

  const _parse_hr = (lns) => {
    return /^(---|___|\*\*\*)\s*$/.test(lns[0]) ? { html: '<hr>', consumed: 1 } : null;
  };
  ...
  const _parse_paragraph = (lns, ctx) => {
    ...
    while (i < lns.length && lns[i].trim() !== '') {
      ...
      if (lns[i].includes('|') && i + 1 < lns.length && lns[i + 1].includes('---')) break;
      if (/^\[\^(.*?)\]:/.test(lns[i].trim())) break;
