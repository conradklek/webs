...
DomNode *dom_create_text_node(const char *text_content) {
  ...
  if (node) {
    node->attributes->as.object->set(
        node->attributes->as.object, "textContent",
        string_value(text_content ? text_content : ""));
  }
  return node;
}
...
void dom_free_node(DomNode *node) {
  ...
  if (children_to_free && children_to_free->type == VALUE_ARRAY) {
    for (size_t i = 0; i < children_to_free->as.array->count; i++) {
      Value *child_ptr_val = children_to_free->as.array->elements[i];
      if (child_ptr_val && child_ptr_val->type == VALUE_POINTER) {
        dom_free_node((DomNode *)child_ptr_val->as.pointer);
      }
    }
  }
...
void dom_append_child(DomNode *parent, DomNode *child) {
  if (!parent || !child)
    return;
  child->parent = parent;
  parent->children->as.array->push(parent->children->as.array, pointer(child));
}
...
void dom_set_attribute(DomNode *node, const char *key, Value *value) {
  ...
  if (node->attributes && node->attributes->type == VALUE_OBJECT) {
    node->attributes->as.object->set(node->attributes->as.object, key, value);
  } else {
    if (value)
      value_free(value);
  }
...
static bool node_matches_selector(DomNode *node, const char *selector) {
  ...
  if (*s == '#') {
    Value *id_attr =
        node->attributes->as.object->get(node->attributes->as.object, "id");
    return id_attr && id_attr->type == VALUE_STRING &&
           strcmp(id_attr->as.string->chars, s + 1) == 0;
  } else if (*s == '.') {
    Value *class_attr =
        node->attributes->as.object->get(node->attributes->as.object, "class");
    if (class_attr && class_attr->type == VALUE_STRING) {
      const char *class_list = class_attr->as.string->chars;
...
static void find_all_nodes(DomNode *start_node, const char *selector,
  ...
  if (node_matches_selector(start_node, selector)) {
    results_array->as.array->push(results_array->as.array, pointer(start_node));
  }

  if (start_node->children && start_node->children->type == VALUE_ARRAY) {
    for (size_t i = 0; i < start_node->children->as.array->count; i++) {
      Value *child_ptr = start_node->children->as.array->elements[i];
      find_all_nodes((DomNode *)child_ptr->as.pointer, selector, results_array);
    }
  }
}
...
Value *dom_query_selector(DomNode *root, const char *selector) {
  ...
  if (results->as.array->count > 0) {
    Value *first_match = value_clone(results->as.array->elements[0]);
    value_free(results);
    return first_match;
  }
...
void dom_add_event_listener(DomNode *node, const char *event_type,
  ...
  Object *listeners_obj = node->event_listeners->as.object;
  ...
  listeners_for_type->as.array->push(listeners_for_type->as.array,
