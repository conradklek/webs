...
void track(Engine *engine, const Value *target, const char *key) {
  ...
  webs()->log->debug("TRACK: target=%p, key='%s'", (const void *)target, key);
  ...
  if (!deps_map_val) {
    ...
    deps_map = (Map *)deps_map_val->as.pointer;
  }
  ...
  if (!dep_list_val) {
    ...
    dep_list = (EffectDepList *)dep_list_val->as.pointer;
  }
...
void trigger(Engine *engine, const Value *target, const char *key) {
  webs()->log->debug("TRIGGER: target=%p, key='%s'", (const void *)target, key);
  ...
  if (!deps_map_val)
    return;
  Map *deps_map = (Map *)deps_map_val->as.pointer;

  Value *dep_list_val = deps_map->get(deps_map, key);
  if (!dep_list_val)
    return;
  EffectDepList *dep_list = (EffectDepList *)dep_list_val->as.pointer;

  EffectDepNode *current = dep_list->head;
  while (current) {
    if (current->effect != engine->active_effect && current->effect->active) {
      webs()->log->debug("Queueing effect %p due to trigger",
                         (void *)current->effect);
...
void ref_set_value(Engine *engine, Value *ref_value, Value *new_value) {
  ...
  Value *old_value = ref_value->as.ref->value;
  if (value_compare(old_value, new_value) != 0) {
    value_free(old_value);
    ref_value->as.ref->value = new_value;
