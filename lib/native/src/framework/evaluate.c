...
static bool is_truthy(const Value *value) {
  ...
  case VALUE_NUMBER:
    return value->as.number_val != 0;
  case VALUE_STRING:
    return value->as.string_val && value->as.string_val->length > 0;
  case VALUE_OBJECT:
  case VALUE_ARRAY:
    return true;
  default:
...
static Value *evaluate_literal(const Value *node, const Value *scope) {
  const Value *literal_value =
      node->as.object_val->get(node->as.object_val, "value");
  return value_clone(literal_value);
}

static Value *evaluate_identifier(const Value *node, const Value *scope) {
  const char *name = node->as.object_val->get(node->as.object_val, "name")
                         ->as.string_val->chars;
  if (scope && scope->type == VALUE_OBJECT) {
    Value *result = scope->as.object_val->get(scope->as.object_val, name);
    if (result) {
...
static Value *evaluate_binary_expression(const Value *node,
                                         const Value *scope) {
  const Object *node_obj = node->as.object_val;
  const char *op = node_obj->get(node_obj, "operator")->as.string_val->chars;
  ...
  if (left->type == VALUE_NUMBER && right->type == VALUE_NUMBER) {
    double l = left->as.number_val;
    double r = right->as.number_val;
    if (strcmp(op, "+") == 0)
      result = number(l + r);
    else if (strcmp(op, "-") == 0)
      result = number(l - r);
...
static Value *evaluate_unary_expression(const Value *node, const Value *scope) {
  ...
  if (strcmp(op, "!") == 0) {
    ...
    if (argument->type == VALUE_NUMBER) {
      result = number(-argument->as.number_val);
    }
  }
...
static Value *evaluate_member_expression(const Value *node,
  ...
  if (object->type != VALUE_OBJECT) {
    ...
    if (is_optional && is_optional->as.boolean_val &&
  ...
  const char *prop_name =
      property_node->as.object_val->get(property_node->as.object_val, "name")
          ->as.string_val->chars;
  Value *result = object->as.object_val->get(object->as.object_val, prop_name);
...
static Value *evaluate_computed_member_expression(const Value *node,
  ...
  if (object->type == VALUE_ARRAY && property->type == VALUE_NUMBER) {
    size_t index = (size_t)property->as.number_val;
    Value *item = array_get(object->as.array_val, index);
    if (item) {
      result = value_clone(item);
    }
  } else if (object->type == VALUE_OBJECT && property->type == VALUE_STRING) {
    Value *item = object->as.object_val->get(object->as.object_val,
                                             property->as.string_val->chars);
    if (item) {
      result = value_clone(item);
    }
  }
...
static Value *evaluate_array_literal(const Value *node, const Value *scope) {
  const Value *elements_ast =
      node->as.object_val->get(node->as.object_val, "elements");
  ...
  for (size_t i = 0; i < elements_ast->as.array_val->count; i++) {
    const Value *element_ast = elements_ast->as.array_val->elements[i];
    Value *element_value = evaluate_node(element_ast, scope);
    if (!element_value) {
      element_value = undefined();
    }
    result_array->as.array_val->push(result_array->as.array_val, element_value);
  }
...
static Value *evaluate_object_literal(const Value *node, const Value *scope) {
  const Value *properties_ast =
      node->as.object_val->get(node->as.object_val, "properties");
  ...
  for (size_t i = 0; i < properties_ast->as.array_val->count; i++) {
    const Value *prop_ast = properties_ast->as.array_val->elements[i];
    const Object *prop_obj = prop_ast->as.object_val;
    const Value *key_ast = prop_obj->get(prop_obj, "key");
    const Value *value_ast = prop_obj->get(prop_obj, "value");

    const char *key_str = NULL;
    const Object *key_ast_obj = key_ast->as.object_val;
    const Value *key_ast_type = key_ast_obj->get(key_ast_obj, "type");
    const char *key_type_str = key_ast_type->as.string_val->chars;

    if (strcmp(key_type_str, "Identifier") == 0) {
      key_str = key_ast_obj->get(key_ast_obj, "name")->as.string_val->chars;
      ...
      if (literal_val->type == VALUE_STRING) {
        key_str = literal_val->as.string_val->chars;
      }
    }
    ...
    if (key_str) {
      ...
      result_object->as.object_val->set(result_object->as.object_val, key_str,
                                        prop_value);
    }
  }
...
static Value *evaluate_node(const Value *node, const Value *scope) {
  ...
  const Value *type_value =
      node->as.object_val->get(node->as.object_val, "type");
  if (!type_value || type_value->type != VALUE_STRING)
    return NULL;

  const char *type = type_value->as.string_val->chars;
