...
static Value *parse_primary(Parser *p) {
  ...
  case TOKEN_NUMBER:
    ...
    node->as.object_val->set(node->as.object_val, "value",
  ...
  case TOKEN_NULL:
    consume(p);
    node = new_ast_node("Literal");
    node->as.object_val->set(node->as.object_val, "value", null());
    return node;
  case TOKEN_UNDEFINED:
    consume(p);
    node = new_ast_node("Literal");
    node->as.object_val->set(node->as.object_val, "value", undefined());
    return node;
  case TOKEN_IDENTIFIER:
    consume(p);
    node = new_ast_node("Identifier");
    node->as.object_val->set(node->as.object_val, "name",
                             string_value(token.value));
    return node;
  case TOKEN_LPAREN:
    consume(p);
...
static Value *parse_array_literal(Parser *p) {
  ...
  node->as.object_val->set(node->as.object_val, "elements", elements);
  ...
  if (peek(p).type != TOKEN_RBRACKET) {
    do {
      ...
      elements->as.array_val->push(elements->as.array_val, element);
...
static Value *parse_object_literal(Parser *p) {
  ...
  node->as.object_val->set(node->as.object_val, "properties", properties);
  ...
  if (peek(p).type != TOKEN_RBRACE) {
    do {
      ...
      if (key_token.type == TOKEN_IDENTIFIER) {
        ...
        key_node->as.object_val->set(key_node->as.object_val, "name",
                                     string_value(key_token.value));
      } else if (key_token.type == TOKEN_STRING) {
        key_node = new_ast_node("Literal");
        key_node->as.object_val->set(key_node->as.object_val, "value",
                                     string_value(key_token.value));
      } else {
      ...
      prop_node->as.object_val->set(prop_node->as.object_val, "key", key_node);
      ...
      prop_node->as.object_val->set(prop_node->as.object_val, "value",
                                    value_node);
      properties->as.array_val->push(properties->as.array_val, prop_node);
    } while (peek(p).type == TOKEN_COMMA && (consume(p), true));
  }
...
static Value *parse_accessors(Parser *p) {
  ...
  while (true) {
    ...
    if (token.type == TOKEN_DOT ||
      ...
      Object *new_node_obj = new_node->as.object_val;
      new_node_obj->set(new_node_obj, "object", node);

      Value *prop_node = new_ast_node("Identifier");
      prop_node->as.object_val->set(prop_node->as.object_val, "name",
      ...
      if (peek(p).type != TOKEN_RPAREN) {
        do {
          ...
          args->as.array_val->push(args->as.array_val, arg);
        } while (peek(p).type == TOKEN_COMMA && (consume(p), true));
      }
      if (peek(p).type != TOKEN_RPAREN) {
...
static Value *parse_binary(Parser *p, Value *(*next_parser)(Parser *),
  ...
  while (token.type == TOKEN_OPERATOR) {
    ...
    Object *new_left_obj = new_left->as.object_val;
