...
static void encode_wson_value(const Value *value, StringBuilder *sb) {
  ...
  case VALUE_REF: {
    sb_append_str(sb, "{\""
                      "$$type"
                      "\":\"ref\",\"value\":");
    encode_wson_value(value->as.ref_val->value, sb);
    sb_append_char(sb, '}');
    break;
  }
  case VALUE_VNODE: {
    sb_append_str(sb, "{\""
                      "$$type"
                      "\":\"vnode\",\"component\":\"");
    sb_append_str(sb, value->as.vnode_val->type);
    sb_append_str(sb, "\"}");
    break;
  }
  case VALUE_OBJECT:
...
static Value *revive_wson_tree(Engine *engine, Value *value) {
  ...
  if (value->type == VALUE_OBJECT) {
    Value *type_tag = value->as.object_val->get(value->as.object_val, "$$type");
    if (type_tag && type_tag->type == VALUE_STRING) {
      const char *type = type_tag->as.string_val->chars;
      if (strcmp(type, "ref") == 0) {
        Value *inner_value =
            value->as.object_val->get(value->as.object_val, "value");
        Value *revived_inner =
    ...
    Map *table = value->as.object_val->map;
    for (size_t i = 0; i < table->capacity; ++i) {
    ...
    for (size_t i = 0; i < value->as.array_val->count; ++i) {
      Value *original_element = value->as.array_val->elements[i];
      Value *revived_element = revive_wson_tree(engine, original_element);
      if (revived_element != original_element) {
        value->as.array_val->elements[i] = revived_element;
      }
    }
  }
  return value;
