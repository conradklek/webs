#include "fetch.h"
#include "../core/console.h"
#include "../core/number.h"
#include "../core/object.h"
#include "../core/string.h"
#include "json.h"
#include <arpa/inet.h>
#include <ctype.h>
#include <errno.h>
#include <netdb.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/socket.h>
#include <unistd.h>

static void set_fetch_error(char **error, const char *msg) {
  if (error && msg) {
    if (*error == NULL) {
      *error = strdup(msg);
    }
  }
}

typedef struct {
  char *host;
  char *path;
  int port;
} ParsedUrl;

static ParsedUrl *parse_url_for_fetch(const char *url, char **error) {
  char *url_copy = strdup(url);
  if (!url_copy) {
    set_fetch_error(error, "Memory allocation failed for URL parsing.");
    return NULL;
  }

  char *original_url_copy_ptr = url_copy;

  const char *scheme_separator = "://";
  char *scheme_ptr = strstr(url_copy, scheme_separator);
  if (!scheme_ptr) {
    set_fetch_error(error, "Invalid URL: scheme missing.");
    free(original_url_copy_ptr);
    return NULL;
  }

  *scheme_ptr = '\0';
  char *scheme = url_copy;

  int port;
  if (strcmp(scheme, "http") == 0) {
    port = 80;
  } else if (strcmp(scheme, "https") == 0) {
    set_fetch_error(error, "HTTPS is not supported.");
    free(original_url_copy_ptr);
    return NULL;
  } else {
    set_fetch_error(error, "Unsupported scheme.");
    free(original_url_copy_ptr);
    return NULL;
  }

  char *host_start = scheme_ptr + strlen(scheme_separator);
  char *path_start = strchr(host_start, '/');
  char *host_end = path_start ? path_start : host_start + strlen(host_start);

  char *port_ptr = strchr(host_start, ':');
  if (port_ptr && port_ptr < host_end) {
    *port_ptr = '\0';
    port = atoi(port_ptr + 1);
  }
  *host_end = '\0';

  ParsedUrl *parsed = malloc(sizeof(ParsedUrl));
  if (!parsed) {
    free(original_url_copy_ptr);
    return NULL;
  }

  parsed->host = strdup(host_start);
  parsed->path = path_start ? strdup(path_start) : strdup("/");
  parsed->port = port;

  free(original_url_copy_ptr);
  return parsed;
}

static void free_parsed_url(ParsedUrl *parsed) {
  if (parsed) {
    free(parsed->host);
    free(parsed->path);
    free(parsed);
  }
}

char *webs_fetch_sync(const char *url, const char *options_json, char **error) {
  ParsedUrl *parsed_url = parse_url_for_fetch(url, error);
  if (!parsed_url) {
    return NULL;
  }

  Value *options = NULL;
  const char *method = "GET";
  const char *body = "";
  char *result_json = NULL;
  char *response_buffer = NULL;
  char *header_buf = NULL;

  if (options_json && strlen(options_json) > 0) {
    Status status;
    options = json_decode(options_json, &status);
    if (status != OK) {
      char err_buf[512];
      snprintf(err_buf, sizeof(err_buf), "Failed to parse options JSON: %s",
               webs_status_to_string(status));
      set_fetch_error(error, err_buf);
      goto cleanup;
    }
    if (options && options->type == VALUE_OBJECT) {
      Object *opts_obj = options->as.object_val;
      Value *method_val = opts_obj->get(opts_obj, "method");
      if (method_val && method_val->type == VALUE_STRING) {
        method = method_val->as.string_val->chars;
      }
      Value *body_val = opts_obj->get(opts_obj, "body");
      if (body_val && body_val->type == VALUE_STRING) {
        body = body_val->as.string_val->chars;
      }
    }
  }

  struct hostent *server = gethostbyname(parsed_url->host);
  if (server == NULL) {
    set_fetch_error(error, "Host not found.");
    goto cleanup;
  }

  int sockfd = socket(AF_INET, SOCK_STREAM, 0);
  if (sockfd < 0) {
    set_fetch_error(error, "Failed to create socket.");
    goto cleanup;
  }

  struct sockaddr_in serv_addr;
  memset(&serv_addr, 0, sizeof(serv_addr));
  serv_addr.sin_family = AF_INET;
  serv_addr.sin_port = htons(parsed_url->port);
  memcpy(&serv_addr.sin_addr.s_addr, server->h_addr_list[0], server->h_length);

  if (connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
    char err_buf[256];
    snprintf(err_buf, sizeof(err_buf), "Connection failed: %s",
             strerror(errno));
    set_fetch_error(error, err_buf);
    close(sockfd);
    goto cleanup;
  }

  size_t content_length = strlen(body);
  int required_size = snprintf(NULL, 0,
                               "%s %s HTTP/1.1\r\n"
                               "Host: %s:%d\r\n"
                               "Content-Length: %zu\r\n"
                               "Connection: close\r\n\r\n",
                               method, parsed_url->path, parsed_url->host,
                               parsed_url->port, content_length) +
                      1;

  header_buf = malloc(required_size);
  if (!header_buf) {
    set_fetch_error(error, "Failed to allocate memory for headers.");
    close(sockfd);
    goto cleanup;
  }

  int header_len = snprintf(header_buf, required_size,
                            "%s %s HTTP/1.1\r\n"
                            "Host: %s:%d\r\n"
                            "Content-Length: %zu\r\n"
                            "Connection: close\r\n\r\n",
                            method, parsed_url->path, parsed_url->host,
                            parsed_url->port, content_length);

  if (send(sockfd, header_buf, header_len, 0) < 0 ||
      (content_length > 0 && send(sockfd, body, content_length, 0) < 0)) {
    set_fetch_error(error, "Failed to send request.");
    close(sockfd);
    goto cleanup;
  }
  free(header_buf);
  header_buf = NULL;

  size_t capacity = 8192;
  response_buffer = malloc(capacity);
  size_t total_read = 0;
  ssize_t n;
  while ((n = recv(sockfd, response_buffer + total_read,
                   capacity - total_read - 1, 0)) > 0) {
    total_read += n;
    if (capacity - total_read < 2) {
      capacity *= 2;
      response_buffer = realloc(response_buffer, capacity);
    }
  }
  response_buffer[total_read] = '\0';
  close(sockfd);

  char *header_end = strstr(response_buffer, "\r\n\r\n");
  if (!header_end) {
    set_fetch_error(error, "Invalid HTTP response: Missing header separator.");
    goto cleanup;
  }
  *header_end = '\0';
  char *response_body = header_end + 4;

  char *saveptr_headers;
  char *status_line = strtok_r(response_buffer, "\r\n", &saveptr_headers);
  if (!status_line) {
    set_fetch_error(error, "Invalid HTTP response: Missing status line.");
    goto cleanup;
  }

  char *saveptr_status;
  strtok_r(status_line, " ", &saveptr_status);
  char *status_code_str = strtok_r(NULL, " ", &saveptr_status);
  char *status_text_str = strtok_r(NULL, "", &saveptr_status);

  Value *result_obj = object_value();
  Object *obj = result_obj->as.object_val;
  obj->set(obj, "status", number(status_code_str ? atoi(status_code_str) : 0));
  obj->set(obj, "statusText",
           string_value(status_text_str ? status_text_str : ""));
  obj->set(obj, "body", string_value(response_body));

  Value *headers_obj = object_value();
  char *header_line = strtok_r(NULL, "\r\n", &saveptr_headers);
  while (header_line) {
    char *colon = strchr(header_line, ':');
    if (colon) {
      *colon = '\0';
      char *value = colon + 1;
      while (*value && isspace((unsigned char)*value))
        value++;
      headers_obj->as.object_val->set(headers_obj->as.object_val, header_line,
                                      string_value(value));
    }
    header_line = strtok_r(NULL, "\r\n", &saveptr_headers);
  }
  obj->set(obj, "headers", headers_obj);

  result_json = json_encode(result_obj);
  value_free(result_obj);

cleanup:
  if (options)
    value_free(options);
  if (response_buffer)
    free(response_buffer);
  if (header_buf)
    free(header_buf);
  free_parsed_url(parsed_url);

  return (*error) ? NULL : result_json;
}
