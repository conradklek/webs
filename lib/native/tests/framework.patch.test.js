...
describe('Webs C Patcher (Diffing)', () => {
  ...
  test('should detect updated event listeners', () => {
    ...
    expect(patches.length).toBe(1);
    expect(patches[0].type).toBe(6); // PATCH_UPDATE_EVENTS
    expect(patches[0].data).toEqual({
      click: 'handlerB',
      mouseover: 'handlerC',
    });
  ...
  describe('Keyed Children', () => {
    test('should add an element in the middle', () => {
      ...
      expect(patch.type).toBe(0);
      expect(patch.path).toEqual([0, 1]);
      expect(patch.data.props.key).toBe('b');
    });
    ...
    test('should move an element forward', () => {
      ...
      expect(patches.length).toBe(1);
      const patch = patches[0];
      expect(patch.type).toBe(5);
      expect(patch.path).toEqual([0]);
      expect(patch.data.length).toBe(1);

      expect(patch.data[0]).toEqual({
        type: 'move',
        from: 1,
        to: 0,
      });
    ...
    test('should reverse a list of children', () => {
      ...
      const patch = patches[0];
      expect(patch.type).toBe(5);
      expect(patch.data.length).toBeGreaterThan(0);
    });

    test('should handle complex reordering (moving to front and back)', () => {
      const n1 = h('div', {}, [
        h('p', { key: 'a' }),
        h('p', { key: 'b' }),
        h('p', { key: 'c' }),
      ...
      expect(patch.data.length).toBeGreaterThan(1);
    });

    test('should handle swapping first and last elements', () => {
      const n1 = h('div', {}, [
        h('p', { key: 'a' }),
        h('p', { key: 'b' }),
        h('p', { key: 'c' }),
