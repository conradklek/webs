import { test, expect, describe, beforeEach, afterEach } from 'bun:test';
import { symbols } from '../bindings.js';
import { dlopen, JSCallback } from 'bun:ffi';
import { resolve } from 'path';

const libPath = resolve(import.meta.dir, '../.webs.dylib');
const lib = dlopen(libPath, symbols);

const {
  webs_engine_api,
  webs_engine_destroy_api,
  webs_json_parse,
  webs_ref,
  webs_ref_get_value,
  webs_ref_set_value,
  webs_effect,
  webs_effect_run,
  webs_effect_stop,
  webs_effect_free,
  webs_free_value,
  webs_scheduler_flush_jobs,
  webs_set_log_level,
} = lib.symbols;

webs_set_log_level(4);

function jsToValuePtr(jsValue) {
  const jsonString = JSON.stringify(jsValue);
  const jsonBuffer = Buffer.from(jsonString + '\0');
  const statusPtr = Buffer.alloc(4);
  const valuePtr = webs_json_parse(jsonBuffer, statusPtr);
  const status = statusPtr.readInt32LE(0);

  if (status !== 0 || !valuePtr || valuePtr.ptr === 0) {
    throw new Error(
      `Failed to parse JS value to WebsValue. Status: ${status}, Value: ${jsValue}`,
    );
  }
  return valuePtr;
}

let enginePtr = null;

beforeEach(() => {
  enginePtr = webs_engine_api();
  if (!enginePtr || enginePtr.ptr === 0) {
    throw new Error('Test setup failed: Could not create WebsEngine.');
  }
});

afterEach(() => {
  if (enginePtr) {
    webs_engine_destroy_api(enginePtr);
    enginePtr = null;
  }
});

describe('Webs C Reactivity Scheduler', () => {
  test('should batch multiple updates into a single effect run', () => {
    let effectRunCount = 0;

    const initialValuePtr = jsToValuePtr(0);
    let counterRefPtr = null;
    let effectPtr = null;
    let effectCallback = null;

    try {
      counterRefPtr = webs_ref(initialValuePtr);

      const effectFn = () => {
        webs_ref_get_value(enginePtr, counterRefPtr);
        effectRunCount++;
      };

      effectCallback = new JSCallback(effectFn, {});
      effectPtr = webs_effect(effectCallback.ptr, null);

      webs_effect_run(enginePtr, effectPtr);
      expect(effectRunCount).toBe(1);

      webs_ref_set_value(enginePtr, counterRefPtr, jsToValuePtr(1));
      webs_ref_set_value(enginePtr, counterRefPtr, jsToValuePtr(2));
      webs_ref_set_value(enginePtr, counterRefPtr, jsToValuePtr(3));

      expect(effectRunCount).toBe(1);

      webs_scheduler_flush_jobs(enginePtr);

      expect(effectRunCount).toBe(2);

      webs_ref_set_value(enginePtr, counterRefPtr, jsToValuePtr(4));

      expect(effectRunCount).toBe(2);

      webs_scheduler_flush_jobs(enginePtr);

      expect(effectRunCount).toBe(3);
    } finally {
      if (counterRefPtr) webs_free_value(counterRefPtr);
      if (effectPtr) webs_effect_free(effectPtr);
      if (effectCallback) effectCallback.close();
    }
  });

  test('should not re-run an effect if the value is set to the same value', () => {
    let effectRunCount = 0;

    const initialValuePtr = jsToValuePtr(10);
    let refPtr = null;
    let effectPtr = null;
    let effectCallback = null;

    try {
      refPtr = webs_ref(initialValuePtr);

      const effectFn = () => {
        webs_ref_get_value(enginePtr, refPtr);
        effectRunCount++;
      };

      effectCallback = new JSCallback(effectFn, {});
      effectPtr = webs_effect(effectCallback.ptr, null);

      webs_effect_run(enginePtr, effectPtr);
      expect(effectRunCount).toBe(1);

      webs_ref_set_value(enginePtr, refPtr, jsToValuePtr(10));
      webs_scheduler_flush_jobs(enginePtr);

      expect(effectRunCount).toBe(1);

      webs_ref_set_value(enginePtr, refPtr, jsToValuePtr(20));
      webs_scheduler_flush_jobs(enginePtr);

      expect(effectRunCount).toBe(2);
    } finally {
      if (refPtr) webs_free_value(refPtr);
      if (effectPtr) webs_effect_free(effectPtr);
      if (effectCallback) effectCallback.close();
    }
  });

  test('should not run a stopped effect', () => {
    let effectRunCount = 0;

    const initialValuePtr = jsToValuePtr(0);
    let refPtr = null;
    let effectPtr = null;
    let effectCallback = null;

    try {
      refPtr = webs_ref(initialValuePtr);

      const effectFn = () => {
        webs_ref_get_value(enginePtr, refPtr);
        effectRunCount++;
      };

      effectCallback = new JSCallback(effectFn, {});
      effectPtr = webs_effect(effectCallback.ptr, null);

      webs_effect_run(enginePtr, effectPtr);
      expect(effectRunCount).toBe(1);

      webs_effect_stop(effectPtr);

      webs_ref_set_value(enginePtr, refPtr, jsToValuePtr(1));
      webs_scheduler_flush_jobs(enginePtr);

      expect(effectRunCount).toBe(1);
    } finally {
      if (refPtr) webs_free_value(refPtr);
      if (effectPtr) webs_effect_free(effectPtr);
      if (effectCallback) effectCallback.close();
    }
  });
});
