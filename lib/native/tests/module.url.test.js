...
describe('Webs C URL Decoder - Full URL Parsing', () => {
  ...
  test('should handle URL with no path', () => {
    const result = parseUrlWithC('ftp://files.example.com?file=archive.zip');
    expect(result).toEqual({
      scheme: 'ftp',
      host: 'files.example.com',
      path: '/',
      query: { file: 'archive.zip' },
    });
  });

  test('should handle URL with no query string', () => {
    const result = parseUrlWithC('https://example.com/some/path#section2');
    expect(result).toEqual({
      scheme: 'https',
      host: 'example.com',
      path: '/some/path',
  ...
  test('should handle URL with no fragment', () => {
    const result = parseUrlWithC('https://example.com/path?name=test');
    expect(result).toEqual({
      scheme: 'https',
      host: 'example.com',
      path: '/path',
  ...
  test('should handle complex query parameters in a full URL', () => {
    const result = parseUrlWithC(
      'http://test.com/search?user[name]=John%20Doe&user[hobbies][]=reading&user[hobbies][]=coding',
    );
    expect(result).toEqual({
      scheme: 'http',
      host: 'test.com',
...
describe('Webs C URL Route Matcher', () => {
  ...
  test('should handle optional catch-all parameters', () => {
    expect(matchRoute('/search/[...query]', '/search/')).toEqual({ query: [] });
    expect(matchRoute('/search/[...query]', '/search')).toEqual({ query: [] });
  });
