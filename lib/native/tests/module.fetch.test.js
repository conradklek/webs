...
describe('Webs C Fetch Module', () => {
  ...
  it('should perform a PUT request', () => {
    ...
    const options = { method: 'PUT', body: putBody };
    const response = fetchWithC(`${serverUrl}/update`, options);
    expect(response.status).toBe(200);
    expect(response.body).toBe(putBody);
  });

  it('should perform a DELETE request', () => {
    const options = { method: 'DELETE' };
    const response = fetchWithC(`${serverUrl}/remove`, options);
    expect(response.status).toBe(200);
  ...
  it('should send custom headers', () => {
    const options = {
      headers: {
        'X-Test-Header': 'Bun FFI',
      },
    };
    const response = fetchWithC(`${serverUrl}/headers`, options);
    expect(response.status).toBe(200);
    const body = JSON.parse(response.body);
    expect(body['x-test-header']).toBe('Bun FFI');
  });

  it('should handle a 204 No Content response', () => {
    const response = fetchWithC(`${serverUrl}/api`, { method: 'OPTIONS' });
