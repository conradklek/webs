...
/** @typedef {import('./renderer.dom.js').Component<any>} Component */
  ...
  const ctx = {
      ...
      switch (expr.type) {
        case 'Identifier':
          return this.scope.has(
            /** @type {import('./expression-evaluator.js').IdentifierNode} */ (
              expr
            ).name,
          )
            ? /** @type {import('./expression-evaluator.js').IdentifierNode} */ (
        ...
        case 'Literal':
          return JSON.stringify(
            /** @type {import('./expression-evaluator.js').LiteralNode} */ (
              expr
            ).value,
          );
        case 'TemplateLiteral': {
          ...
          let i = 0;
          const templateNode =
            /** @type {import('./expression-evaluator.js').TemplateLiteralNode} */ (
              expr
            );
        ...
        case 'ObjectExpression': {
          const props =
            /** @type {import('./expression-evaluator.js').ObjectExpressionNode} */ (
              expr
            ).properties
              .map((p) => {
                const key =
        ...
        case 'ArrayExpression': {
          return `[${
            /** @type {import('./expression-evaluator.js').ArrayExpressionNode} */ (
              expr
            ).elements
        ...
        case 'BinaryExpression':
          const binaryNode =
            /** @type {import('./expression-evaluator.js').BinaryExpressionNode} */ (
              expr
            );
        ...
        case 'UnaryExpression':
          const unaryNode =
            /** @type {import('./expression-evaluator.js').UnaryExpressionNode} */ (
              expr
            );
          return `${unaryNode.operator}${this.genExpr(unaryNode.argument)}`;
        case 'MemberExpression': {
          const memberNode =
            /** @type {import('./expression-evaluator.js').MemberExpressionNode} */ (
              expr
            );
        ...
        case 'ComputedMemberExpression':
          const computedNode =
            /** @type {import('./expression-evaluator.js').ComputedMemberExpressionNode} */ (
              expr
            );
        ...
        case 'CallExpression':
          const callNode =
            /** @type {import('./expression-evaluator.js').CallExpressionNode} */ (
              expr
            );
        ...
        case 'ConditionalExpression':
          const conditionalNode =
            /** @type {import('./expression-evaluator.js').ConditionalExpressionNode} */ (
              expr
            );
        ...
        case 'AssignmentExpression':
          const assignmentNode =
            /** @type {import('./expression-evaluator.js').AssignmentExpressionNode} */ (
              expr
            );
    ...
    genChildren(children) {
      ...
      const childNodes = (Array.isArray(children) ? children : [children])
        .map((/** @type {import('./template-parser.js').HtmlAstNode} */ c) =>
          this.genNode(c),
        )
        .filter((c) => c && c !== 'null');
      ...
      switch (node.type) {
        ...
        case NODE_TYPES.FRAGMENT:
          const fragmentChildren = (
            ...
            .map(
              (/** @type {import('./template-parser.js').HtmlAstNode} */ c) =>
                this.genNode(c),
            )
            .filter(Boolean);
...
export class Compiler {
  ...
  /**
   * @param {import('./template-parser.js').HtmlAstNode[]} children
  ...
  _processSlots(children) {
    ...
    for (const child of children) {
      if (
        child.type === 'element' &&
        /** @type {import('./template-parser.js').ElementNode} */ (child)
          .tagName === 'template'
      ) {
        const slotAttr =
          /** @type {import('./template-parser.js').ElementNode} */ (
            child
          ).attributes.find((/**@type {{name: string}}*/ a) =>
            a.name.startsWith('#'),
          );
        if (slotAttr) {
      ...
      if (
        child.type === 'text' &&
        !(
          /** @type {import('./template-parser.js').TextNode} */ (
            child
          ).content.trim()
        )
      ) {
  ...
  /**
   * @param {import('./template-parser.js').HtmlAstNode} node
   * @returns {any}
   */
  _transformNode(node) {
    switch (node.type) {
      ...
      case 'comment':
        return {
          ...
          value: /** @type {import('./template-parser.js').CommentNode} */ (
            node
          ).content,
        };
      case 'ifBlock':
   ...
   */
  _transformText(node) {
    const text = /** @type {import('./template-parser.js').TextNode} */ (node)
      .content;
    if (!text.includes('{{')) {
  ...
  _transformChildren(children) {
    ...
    while (i < children.length) {
      ...
      if (child.type === 'ifBlock') {
        ...
        const branches = [];
        /** @type {import('./template-parser.js').HtmlAstNode | undefined} */
        let current = child;
        let nextIndex = i + 1;
        while (current) {
          if (current.type === 'ifBlock' || current.type === 'elseIfBlock') {
            branches.push({
              condition: this._parseExpr(
                /** @type {import('./template-parser.js').IfBlockNode | import('./template-parser.js').ElseIfBlockNode} */ (
                  current
                ).test,
              ),
              node: {
  ...
  _transformBlock(block) {
    if (block.type === 'eachBlock') {
      let itemStr = String(block.item).trim();
