 ...
 * @typedef {import('../renderer/renderer.dom.js').Renderer<any>} Renderer
 */
/**
 * @template T
...
export async function hydrate(componentManifest, dbConfig = null) {
  ...
  const startHydrationProcess = async () => {
    ...
    // The server combines layout and page components into a single route component
    ...
    try {
      ...
      // FIX: The navigation handler was not being installed on hydration.
      installNavigationHandler(app);
