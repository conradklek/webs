...
// We capture the function passed to onMounted so we can manually trigger it in tests.
...
describe('Runtime', () => {
  beforeEach(() => {
    ...
    // Reset the captured callback before each test
  ...
  describe('Exports', () => {
    ...
    test('should re-export lifecycle hooks', () => {
      // Note: onMounted and onUnmounted are mocked above for testing purposes
  ...
  describe('table()', () => {
    ...
    test('should initialize and fetch data on mount', async () => {
      ...
      const myTable = runtime.table('test', []); // initialData is []

      // Manually trigger the onMounted lifecycle hook that was captured by our mock
      if (mountedCallback) {
      ...
      expect(getAllMock).toHaveBeenCalled(); // This should now pass
