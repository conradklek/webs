/**
 * @file Manages the client-side IndexedDB database, including setup, transactions, and data access.
 */
...
export const coreDB = {
  ...
  put: async (tableName, record) => {
    const isSynced = /** @type {any} */ (
      window
    ).__WEBS_DB_CONFIG__?.clientTables.find(
      (/** @type {import('./db.client.js').TableSchema} */ t) =>
        t.name === tableName,
    );
...
/**
 * A wrapper around `coreDB.performTransaction` for external use.
 * @template T
...
/**
 * @internal
 * Creates a mock DB interface for server-side rendering (SSR) environments.
 * @returns {DbTableApi}
 */
 ...
 * @property {(prefix: string) => Promise<any[] | undefined>} getAllWithPrefix - Retrieves records whose primary key starts with a given prefix.
