...
async function executePrefetch(routeDefinition, context) {
  ...
  if (!routeDefinition.component?.actions?.prefetch) {
    logger.warn('No prefetch action found.');
    return {};
  }
  try {
    const result = await routeDefinition.component.actions.prefetch(context);
...
export function createFetchHandler(context) {
  ...
  return async function fetch(req, server) {
    ...
    if (req.headers.get('upgrade') === 'websocket') {
      if (pathname === '/api/sync' && extendedReq.user)
      ...
      if (pathname === '/api/chat')
        return context.ai.handleChatUpgrade(
          /** @type {Request & { user: any }} */ (req),
        );
    ...
    if (actionMatch) {
      ...
      const routeDef = Object.values(appRoutes).find(
        (
          /** @type {{ componentName: string; component: { name: string; }; }} */ r,
        ) =>
          r.componentName === componentName ||
          r.component.name === componentName,
      );
      const actions = routeDef?.component?.actions;
      const action = actions ? actions[actionName] : undefined;
        ...
        const result = await action(
          {
            req,
            db,
            fs: createFileSystemForUser(extendedReq.user.id, db),
